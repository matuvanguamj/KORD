/**
 * KORD LOADER — Machine à écrire lente + barre de chargement
 * S'affiche uniquement sur : index.html et après connexion (dashboard.html)
 */
(function() {

  // Pages où le loader s'affiche
  const path = window.location.pathname;
  const showOn = [
    '/', '/index.html', '/kord.html',
    '/dashboard.html', '/admin.html'
  ];

  // Sur dashboard/admin — seulement si on vient de se connecter
  ...
  if (!isIndex && !isDashboard) {
    // login, register, etc. → pas de loader
    addTransitionOnly();
    return;
  }
  ...
  // Délais intentionnellement longs — illusion de chargement
  const WORD   = 'KORD';
  const DELAYS = [0, 380, 820, 1340]; // K...O..R.D — de plus en plus court
  ...
  function startTypewriter() {
    ...
    WORD.split('').forEach((char, i) => {
      setTimeout(() => {
        ...
        // Avancer la barre proportionnellement
      ...
      }, 400 + DELAYS[i]); // 400ms avant la 1ère lettre
    });
  }
  ...
  // Durée minimale = fin de l'animation de typing (~2.2s)
  const MIN   = 2400;
  ...
  // Marquer la connexion pour que dashboard affiche le loader
...
// ── Version sans loader (login, register) ──
